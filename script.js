...
const ROWS = 20;
const BLOCK_SIZE = 30;
const SHAPES = [ /* как ранее */ ];

// Классы Tetromino, Board (без изменений, кроме мелких правок)
// ... (см. предыдущий код, они корректны)
...
class Game {
    ...
    lockPiece() {
        ...
        // обновляем интервал, если уровень изменился
        if (this.interval) {
            this.stop();
    ...
    continueAfterAd() {
        if (this.boardSnapshot) {
            ...
            // удаляем одну заполненную строку сверху (для освобождения места)
